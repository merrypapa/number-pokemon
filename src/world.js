...
export function buildWorld(scene) {
  ...
  return {
    ...
    npcs: [{ x: totoroAt.x, z: totoroAt.z, mesh: totoro, name: '도토로', lines: (c) => [
    ...
    ] }, { x: bb.x, z: bb.z, mesh: bugs, name: '벅스버니', lines: (c) => [
      `어이, 무슨 일이야 ${c.name}? 난 벅스버니. 당근 씹으면서 이 숲의 수수께끼 상자를 지키고 있지.`,
      '땅에 떠 있는 하얀 블록들 사이에 가끔 반짝이는 상자가 섞여 있어. 그게 수수께끼 상자야!',
      '상자를 주우면 블록 대신 숫자블록 친구가 톡 튀어나와. 문제를 맞히면 그 친구 숫자만큼 블록을 한꺼번에 주지. 열이가 나오면 열 개!',
      '물의길이랑 심해에서는 상자가 바다 보물상자 모양으로 나와. 열면 똑같이 친구가 나오니까 꼭 주워 보라고.',
    ] }, { x: 4.5, z: 15, mesh: ranger, name: '나미', warp: true, lines: (c) => [
      `안녕, ${c.name}! 난 푸른숲 안내원 나미야. 여기 포켓몬은 공격 ${c.zone.atkRange} 정도면 편하게 이길 수 있어.`,
      '요즘 숲이 뒤숭숭해. 검은 옷에 빨간 R 을 단 사람들이 지나갔고, 그 뒤로 포켓몬들 머리에 붉은 숫자가 떠 있어. 넘버로켓단이래.',
      c.conquered.forest ? '네가 이상해꽃 머리의 붉은 60을 깨뜨려 줬지. 이제 눈이 웃는 걸 봤어!' : '아레나의 이상해꽃도 붉은 60을 뒤집어썼어. 저 숫자를 0으로 깎아 주면 우리 숲 친구로 돌아올 거야.',
      ...
      c.conquered.forest ? '푸른숲 보스 이상해꽃은 이미 네 친구! 북쪽 산의 동굴 입구가 열렸어. 지하동굴에 가 보자.' : `서북쪽 돌기둥 아레나에 보스 이상해꽃이 있어. 체력 60에 공격 6이나 되니까 공격 ${c.zone.targetAtk + 4} 이상, 체력 25쯤 되면 도전해 봐. 불 포켓몬이면 더 좋아!`,
      ...
      '난 원래 항해사야. 깊은 바다에도 자주 내려가니까, 심해에 가면 거기서도 나를 찾아와!',
