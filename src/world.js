...
export function buildWorld(scene) {
  ...
  scene.fog = new THREE.Fog(0x8fd3ff, 120, 330);
  ...
  // ---------- 벅스버니 (지역 안내 NPC): 시작 지점 옆. 대화 버튼으로 이야기, 말하는 동안 연구소로 데려다준다 ----------
  // 예전에는 숲지기 나미가 이 자리였는데, 나미는 항해사라 심해로 갔다 (src/deepsea.js)
  const bugs = makeNpc({ outfit: 'ranger', name: '벅스버니', model: '벅스버니.glb' });
  const bb = { x: 4.5, z: 15 };
  bugs.position.set(bb.x, meadowHeight(bb.x, bb.z), bb.z);
  bugs.rotation.y = -0.6; // 시작 지점 쪽을 본다
  decor.add(bugs); block(bb.x, bb.z, 0.6);
  ...
  return {
    ...
    npcs: [{ x: totoroAt.x, z: totoroAt.z, mesh: totoro, name: '도토로', lines: (c) => [
      ...
      '로켓단 포켓몬은 명령만 들어서 기술이 뻔해. 같이 자란 친구가 훨씬 세다고!',
      '상대 속성을 잘 봐. 불은 풀에, 물은 불에, 풀은 물에 세! 대결 중 "교체하기"로 잘 맞는 포켓몬을 내보낼 수 있어.',
      c.conquered.forest ? '이상해꽃을 이겼다니 대단해! 다음엔 다른 지역 보스한테 도전해 봐. 정복할수록 넘버볼 만들 블록도 많이 생겨.' : `아레나 보스 이상해꽃은 체력 60에 공격 6이야. 공격 ${c.zone.targetAtk + 4} 이상, 체력 25쯤으로 키우고, 볼도 넉넉히 만들어서 가자!`,
      '체력이 0이 된 포켓몬은 마을 남쪽 연구소에서 오박사님이 치료해 줘. 무리하지 말고 다녀와!',
    ] }, { x: bb.x, z: bb.z, mesh: bugs, name: '벅스버니', warp: true, lines: (c) => [
      `어이, 무슨 일이야 ${c.name}? 난 벅스버니. 이 숲 안내는 이제 내 담당이라고. 여기 포켓몬은 공격 ${c.zone.atkRange} 정도면 가볍지.`,
      '요즘 숲이 뒤숭숭해. 검은 옷에 빨간 R 을 단 녀석들이 지나갔고, 그 뒤로 포켓몬들 머리에 붉은 숫자가 떠 있어. 넘버로켓단이래.',
      '그중 한 녀석이 아직 동북쪽에 남아 있어. 붉은 바위 협곡으로 가는 길목에 검은 옷을 입고 떡하니 서 있다고. 말을 걸면 세뇌한 포켓몬을 내보낼 테니, 그 붉은 숫자를 0으로 깎아 줘!',
      c.conquered.forest ? '네가 이상해꽃 머리의 붉은 60을 깨뜨려 줬지. 눈이 웃는 걸 봤다니까!' : '아레나의 이상해꽃도 붉은 60을 뒤집어썼어. 저 숫자를 0으로 깎아 주면 우리 숲 친구로 돌아올 거야.',
      '하얀 블록을 줍거나 대결에서 이기면 블록이 생겨. 도감에서 블록으로 포켓몬을 키우라고. 숫자블록 친구가 도와달라고 하면 문제를 풀어 줘, 블록을 왕창 주거든!',
      '땅에 떠 있는 하얀 블록들 사이에 가끔 반짝이는 상자가 섞여 있어. 그게 수수께끼 상자야!',
      '상자를 주우면 숫자블록 친구가 톡 튀어나와서 바로 문제를 내. 맞히면 상자 속 블록을 한꺼번에 주는데, 몇 개인지는 풀고 나서야 알 수 있어. 가끔 열 개가 넘게 나온다고!',
