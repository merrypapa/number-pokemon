...
- 할 수 있는 것: 시작할 때 내 포켓몬 고르기(피카츄·파이리·이상해씨·꼬부기), 초원(180×180) 탐험(마을·흙길·연못·산. 나무·집·바위 같은 구조물은 지나갈 수 없고, 연못과 지하 호수는 나무 다리로만 건넌다), 하얀 블록 줍기(주운 블록이 숫자블록이 되어 따라옴), 몬스터 6종 21마리와 포켓몬 대결(내 대표 포켓몬이 기술로 공격 → 상대가 반격 → 상대 체력 0이면 숫자볼로 캡처), 도감(B)에서 대표 포켓몬 고르기·블록으로 공격력/체력 올리기·진화(조건을 채우면 버튼이 켜짐)·친구/구출/보스 진행 보기, 둘이·셋이 구출, 서북쪽 아레나의 거대 보스 쿵쿵이(체력 30), 큰 구멍이나 열린 동굴 입구로 들어가는 괴물 동굴(120×120, 달빛처럼 은은히 밝고 대결 중엔 무대 조명이 켜짐. 몬스터 3종 9마리, 수정, 지하 호수, 포니타를 잡으면 밝아짐)과 숲마을로 돌아오는 포탈.

## 포켓몬 대결 규칙 (요약)

1. **내 포켓몬**: 시작할 때 넷 중 하나를 고른다. 잡은 포켓몬은 모두 "내 포켓몬"이 되지만, 주인공 뒤를 따라다니며 싸우는 건 **대표 한 마리**뿐이다. 대표는 도감(B)에서 바꾼다.
2. **스탯**: 포켓몬마다 공격력(⚔)과 체력(❤)이 있다. 야생 몬스터도 같은 스탯으로 싸운다(`data/creatures.json` 의 `baseHp`, `baseAtk`).
3. **숫자블록 = 성장**: 주운 블록 1개로 공격력 +1 또는 체력 +1 (도감에서). 블록 20개까지 들 수 있다.
4. **기술**: 공격력이 오르면 기본 기술(몸통박치기)의 피해가 1 → 2 → 3… 커지고, 공격력 10·20이 되면 새 기술(피카츄: 전기쇼크 ×1.5, 백만볼트 ×2)이 열린다.
5. **반격과 패배**: 내가 공격하면 상대가 반격해 내 체력을 상대 공격력만큼 깎는다. 내 체력이 0이면 진다. 지면 체력이 그 포켓몬의 기본 체력으로 돌아가고(공격력은 그대로), 블록을 모아 다시 올린다. 이기면 체력이 가득 찬다. 탐험 중엔 3초에 1씩 회복.
6. **진화**: 공격력 10·체력 15가 되면 도감에 ✨ 진화! 버튼이 나온다 (피카츄 → 라이츄, 파이리 → 리자드, 이상해씨 → 이상해풀, 꼬부기 → 어니부기). 진화하면 스탯 보너스와 새 기술(공격 30)이 생긴다.
...
| `src/main.js` | 게임 루프, 상태, 시작 포켓몬 고르기, 대표 따라오기, 튜토리얼 |
| `src/world.js` | 초원 지형(언덕·구멍·나무·꽃), 나무 다리, 물 막기(`isBlocked`)와 구조물 충돌(`resolveObstacles`) |
