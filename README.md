...
- 할 수 있는 것:
  ...
  - **지역 5곳**: 푸른숲(시작 마을, 220×220, 풀·노말·벌레·전기) · 지하동굴(160×160, 땅·바위·독, 어둡고 호수·다리·빛나는 웅덩이) · 불의산(160×160, 불, 용암 화산) · 물의길(180×180, 물, 다리로 이어진 모래섬) · 꿈의우주(160×160, 페어리·에스퍼·고스트, 별하늘과 약한 중력). 포켓몬은 속성에 맞는 지역에 산다.
  - **지역 이동**: 푸른숲 큰 구멍/북쪽 산 입구 → 지하동굴, 동북쪽 붉은 바위 아치 → 불의산, 서쪽 기차역에서 E → 기차 타고 물의길, 남동쪽 로켓 발사장에서 E → 로켓 타고 꿈의우주. 각 지역의 빛나는 포탈로 푸른숲에 돌아온다.
  - **보스와 정복**: 지역마다 크고 센 보스(이상해꽃·롱스톤·리자몽·거북왕·뮤)가 있다. 보스를 잡으면 그 지역 정복(HUD ★, 도감 "정복 n/5"). 푸른숲 보스를 잡으면 지하동굴 입구 바위가 치워진다.
  - **포켓몬 대결**: 내 대표 포켓몬이 기술로 공격 → 상대가 반격 → 상대 체력 0이면 숫자볼로 캡처. 도감(B)에서 대표 고르기·블록으로 공격력/체력 올리기·진화. 같은 종을 여러 번 잡으면 도감에 마리 수가 쌓이고, 진화하려면 공격 10·체력 15에 더해 그 종을 3마리 잡아야 한다.
...
| `src/world.js` | 푸른숲 지형(언덕·구멍·나무·꽃·마을), 불의산 입구·기차역·로켓 발사장, 나무 다리, 물 막기(`isBlocked`)와 구조물 충돌(`resolveObstacles`), 지역 공용 헬퍼(바닥·포탈·표지판) |
