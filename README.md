...
- 할 수 있는 것:
  ...
  - **도감 지도 탭**: 다섯 지역을 그림 지도로 보여준다. 정복한 지역은 색과 ★, 아닌 곳은 회색. 지역을 누르면 설명·가는 길·그 지역 포켓몬(잡은 것은 컬러, 못 잡은 것은 실루엣, 보스 표시)이 나온다.
  ...
  - **포켓몬 대결**: 내 대표 포켓몬이 기술로 공격 → 상대가 반격 → 상대 체력 0이면 숫자볼로 캡처. 도감(B)은 내 포켓몬 칩과 선택한 한 마리의 상세(스탯·기술·진화·키우기·대표·진화 버튼) + 아래 액자 그리드(액자를 누르면 상세가 바뀜). 같은 종을 여러 번 잡으면 도감에 마리 수가 쌓이고, 진화하려면 공격 10·체력 15에 더해 그 종을 3마리 잡아야 한다.
