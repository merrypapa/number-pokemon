...
- 할 수 있는 것: 시작할 때 내 포켓몬 고르기(피카츄·파이리·이상해씨·꼬부기), 초원 탐험(마을·흙길·연못·산. 나무·집·바위 같은 구조물은 지나갈 수 없고, 연못과 지하 호수는 나무 다리로만 건넌다), 하얀 블록 줍기(주운 블록이 숫자블록이 되어 따라옴), 몬스터 6종 15마리와 포켓몬 대결(내 대표 포켓몬이 기술로 공격 → 상대가 반격 → 상대 체력 0이면 숫자볼로 캡처), 도감(B)에서 대표 포켓몬 고르기·블록으로 공격력/체력 올리기·진화(조건을 채우면 버튼이 켜짐)·친구/구출/보스 진행 보기, 둘이·셋이 구출, 서북쪽 아레나의 거대 보스 쿵쿵이(체력 30), 큰 구멍이나 열린 동굴 입구로 들어가는 어두운 괴물 동굴(몬스터 3종 6마리, 수정, 지하 호수, 반디를 잡으면 밝아짐)과 숲마을로 돌아오는 포탈.
